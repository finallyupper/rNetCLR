...
def parse_args():
    ...
    Accepted arguments:
      ...
      interval -- time interval for making window used in noise injection feature
      fp16_precision     -- fp16_precision
      n_views     -- number of samplings
      method -- Choose augmenation method : [noise injection, add outcoming packet]
    Returns
    -------
    Namespace
    ...
    parser.add_argument("--interval",
                        type=float,
                        default=1.0,
                        help = "time interval for making window used in noise injection feature")
    ...
    parser.add_argument("--method",
                        type = int,
                        help = "Choose augmentation method (0, 1): [noise injection, add outcoming packet]")
...
def main(output_path, logs_name, batch_size = 256, num_epoches=100, temperature=0.5, interval =1.0, fp16_precision=True, n_views = 2, method=0):
    """
    Run the command Starting from the directory : */rNetLR
    
    ex.
    python main.py --output "models/nj_1_e_100_" --logs_name "nj_1_e_100.txt" -b 256 -n 100 -t 0.5 
    model is saves like this: "models/nj_1_e_100_63.pth.tar"  (*63 is count of epochs)
    ...
    logs_path = '/home/yoojinoh/winter_internship/Realistic-Website-Fingerprinting-By-Augmenting-Network-Traces/artifacts/src/rNetCLR/logs/' + \
        logs_name
    
    print(f'model path : {output_path}\nlogs path : {logs_path}')
    print(f'batch: {batch_size}, epochs: {num_epoches}, temp: {temperature}, intv: {interval}, fp16: {fp16_precision}, n_views: {n_views}')
    
    ##### data path fixed ###############################################
    default_dir = '/scratch/DA/dataset/tcp5'
    data_path = os.path.join(default_dir, 'cfg1_pretrain.npz') # 'cfg2_pretrain.npz'
    data = np.load(data_path) 
    print(f'data files loaded : {data.files}')

    x_train = data['X_pretrain_superior'] 
    y_train = data['y_pretrain_superior']
    ...
    print (f'Train data shapes: {x_train.shape}, {y_train.shape}')# (50000, 5000, 2), (50000,)
    ...
    augmentor = Augmentor(logs_path, OUTCOMING_SIZE_CDF, outcoming_sizes, max_outcoming_size, interval, method)
